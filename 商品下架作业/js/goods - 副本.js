...
for( var i = 0; i < len; i++ ){
	str += `<li>
			<h2 class="setTimeH2">
				<input type="text" class="setTimeInp" value="2017 7 13 23:50:20">
...
var setTimeStr = $('.setTimeInp',lis[0])[0].value;

function calculateTime(ele){//倒计时，计算间隔时间
...
var oldTime = calculateTime( lis[0] );//手动运行一次，计算间隔时间，并将返回值赋给oldTime
var ps = $('p',lis[0]);
...
init();
lis[0].timer = setInterval(countDown,1000)
function countDown(){
	var newTime = calculateTime( lis[0] );//距离目标时间最新的差值
	if( newTime.length > 8 ){//判断是否到指定时间,如果到了指定时间，那么清除定时器并执行抖动函数
		newTime.length = '00:00:00';
		var shade = $('.shade',lis[0])[0];
...
//点击确认按钮
var affirmTime = $('.affirmTime',lis[0])[0];
affirmTime.onclick = function(){
	setTimeStr = $('.setTimeInp',lis[0])[0].value;
	countDown();
...
function soldOut(){
	//var ulHtml = '';
	list.innerHTML +=`<li>
						<span class="showName">${goodsData[0].goodsName}</span>
						<span class="showPrice">${goodsData[0].goodsPrice}</span>
						<span class="showImg">${goodsData[0].goodsImg}</span>
					</li>`;
}
